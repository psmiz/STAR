#!/bin/bash
# Combined evaluation script for 182M STAR MoE model.
...
MODEL=star_moe
ITER=${ITER:-60000}
CKPT_ROOT=${CKPT_ROOT:-./logs/star_182m_lr4e4_aux_B64_E8_GLR5e-5}
...
LM_HARNESS_RESULTS=${RESULT_ROOT}/eval_results_star_182m_${ITER}.json
LAMBADA_LOG=${RESULT_ROOT}/eval_lambada_star_182m_${ITER}.log
COMBINED_RESULTS=${RESULT_ROOT}/eval_combined_star_182m_${ITER}.json
...
# Model architecture (must match 182M training config)
MODEL_ARGS=(
    --use-mcore-models
    --disable-bias-linear
    --no-rope-fusion
...
MOE_ARGS=(
    --num-experts 8
    --moe-router-topk 1
    --moe-router-load-balancing-type ${LB_TYPE}
    --moe-aux-loss-coeff ${AUX_COEFF}
    --moe-token-dispatcher-type alltoall
    --moe-router-pre-softmax
    --moe-star-routing
    --moe-star-gha-lr ${GHA_LR_FLAG}
    --moe-grouped-gemm
    --moe-granularity 1
)
...
echo "=========================================="
echo "Combined Evaluation for 182M STAR Model"
echo "=========================================="
