...
def train(
    ...
    while iteration < args.train_iters:
        ...
        if config.moe_relu_routing and hasattr(config, "moe_relu_sparsity"):
        ...
        if (
            ...
            and config.num_moe_experts is not None
            and hasattr(config, "moe_relu_sparsity")
            and hasattr(config, "moe_relu_l1_reg_coeff")
        ):
            config.moe_relu_sparsity /= config.num_layers * num_microbatches
            torch.distributed.all_reduce(
                config.moe_relu_sparsity,
                group=mpu.get_data_parallel_group(),
                op=torch.distributed.ReduceOp.SUM,
            )
            config.moe_relu_sparsity /= torch.distributed.get_world_size(
                group=mpu.get_data_parallel_group()
            )
            torch.distributed.all_reduce(
                config.moe_relu_sparsity,
            ...
            if config.moe_relu_sparsity.item() < target_sparsity:
