...
def _get_checkpoint_format(checkpoint_name, args):
    ...
    if MultiStorageClientFeature.is_enabled():
        ...
        has_distcp_files = any([f.name.endswith(".distcp") for f in checkpoint_dir.iterdir()])
    else:
        ...
        has_distcp_files = any([f.endswith(".distcp") for f in os.listdir(checkpoint_name)])

    ckpt_format = None
    if dist_checkpointing.check_is_distributed_checkpoint(checkpoint_name):
        ckpt_format = "torch_dist"
    elif has_distcp_files:
        ckpt_format = "torch_dist"
    elif is_torch_ckpt:
        ckpt_format = "torch"
    elif is_torch_dcp:
...
def load_args_from_checkpoint(
    ...
    _set_arg('moe_relu_routing', force=True)
    _set_arg('moe_relu_l1_reg_coeff_init', force=True)
    _set_arg('moe_relu_l1_reg_coeff_multiplier', force=True)
    _set_arg('moe_expert_choice_routing', force=True)
    _set_arg('moe_expert_choice_softmax_order', force=True)
