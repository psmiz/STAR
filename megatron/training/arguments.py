...
def _add_moe_args(parser):
    ...
    group.add_argument('--moe-router-padding-for-fp8', action='store_true',
                       ...
                       'GroupedMLP layer.')
    group.add_argument('--moe-star-routing', action='store_true',
                       help='Enable STAR (Adaptive Sparse Mixture of Experts with Gradient-based Assignment) routing.')
    group.add_argument('--moe-star-gha-lr', type=float, default=2e-5,
                       help='Learning rate for the Generalized Hebbian Algorithm (GHA) in STAR routing. Default is 2e-5.')
    group.add_argument('--moe-star-fp32-gate', action='store_true',
                       help='Use FP32 precision for STAR gate computations.')
    ...
    group.add_argument('--moe-relu-l1-reg-coeff-multiplier', type=float, default=1.2,
                       help='Adaptive update multiplier for ReLU routing L1 regularization.')
    group.add_argument('--moe-expert-choice-routing', action='store_true',
                       help='Enable Expert-Choice routing (Zhou et al. 2022). Each expert '
                            'picks its top-k tokens; k = capacity_factor * num_tokens / '
                            'num_experts (set via --moe-expert-capacity-factor). '
                            'Auto-balanced; use --moe-router-load-balancing-type none.')
    group.add_argument('--moe-expert-choice-softmax-order', type=str,
                       choices=['softmax_topk', 'topk_softmax'],
                       default='softmax_topk',
                       help="Order of softmax/topk in EC routing. 'softmax_topk' applies "
                            "softmax over experts then topk over tokens. 'topk_softmax' "
                            "takes topk first, then softmax over the picked logits.")
    ...
    group.add_argument('--moe-granularity', type=int, default=1, dest='moe_upcycling_granularity',
                       help='Alias for --moe-upcycling-granularity.')
