...
from megatron.core.transformer.moe.star import STARGate
...
class ReLURouter(TopKRouter):
    """Route tokens to experts selected by ReLU activations."""
    ...
    def apply_l1_reg(self, probs: torch.Tensor, routing_map: torch.Tensor):
        """Apply adaptive L1 regularization for ReLU routing."""
        if hasattr(self.config, "moe_relu_l1_reg_coeff"):
            l1_reg_coeff = self.config.moe_relu_l1_reg_coeff.item()
        else:
            l1_reg_coeff = self.config.moe_relu_l1_reg_coeff_init
    ...
    def routing(self, logits: torch.Tensor):
        ...
        if self.training and torch.is_grad_enabled():
            probs = self.apply_l1_reg(probs, routing_map)
            if hasattr(self.config, "moe_relu_sparsity"):
                sparsity = 1.0 - routing_map.float().mean()
                self.config.moe_relu_sparsity += sparsity

        return probs, routing_map


class STARRouter(Router):
    """STAR Router with Generalized Hebbian Algorithm for adaptive basis learning."""

    def __init__(
        self, config: TransformerConfig, pg_collection: Optional[ProcessGroupCollection] = None
    ) -> None:
        """Initialize the STAR router.
        ...
        self.star_gate = STARGate(
            ...
            glr=getattr(self.config, 'moe_star_gha_lr', 2e-5)
        )

        if getattr(self.config, 'moe_star_gha_only', False):
            # Linear router weight + alpha gate are unused under GHA-only routing;
            # keep them out of the optimizer / DDP grad-reduce buckets.
            self.weight.requires_grad_(False)
            if self.bias is not None:
                self.bias.requires_grad_(False)
            self.star_gate.alpha.requires_grad_(False)
    ...
    def gating(self, input: torch.Tensor):
        """Forward pass of the STAR gate.

        Args:
            input (torch.Tensor): Input tensor.

        Returns:
            torch.Tensor: Combined logits tensor.
        """
        if self.weight.device.type == 'cpu':
            self.weight.data = self.weight.data.to(device=torch.cuda.current_device())
        if self.bias is not None and self.bias.device.type == 'cpu':
        ...
        adaptive_logits, alpha = self.star_gate(input)
        if getattr(self.config, 'moe_star_gha_only', False):
            return adaptive_logits
        logits = router_gating_linear(input, self.weight, self.bias, router_dtype)
        combined_logits = alpha * logits + (1 - alpha) * adaptive_logits
        return combined_logits

    def routing(self, logits: torch.Tensor):
        """STAR routing function
    ...
    def forward(self, input: torch.Tensor):
        """Forward pass of the STAR router."""
        self._maintain_float32_expert_bias()
...
class ExpertChoiceRouter(TopKRouter):
    """Expert-Choice routing (Zhou et al. 2022, https://arxiv.org/pdf/2202.09368v2.pdf).

    Each expert picks its top-k tokens (where k = capacity_factor * num_tokens / num_experts)
    instead of each token picking its top-k experts. Auto-load-balanced by construction.

    Returns (probs, routing_map) of shape [num_tokens, num_experts] with variable per-token
    cardinality, dispatched via the dynamic-token-count path also used by ReLU routing.
    """
    ...
    def __init__(
        ...
        self.softmax_order = self.config.moe_expert_choice_softmax_order

    def _k_per_expert(self, num_tokens: int) -> int:
        # k recomputed per call so it tracks varying micro-batch / seq length / recompute.
        k = int(self.capacity_factor * num_tokens / self.num_experts)
        return max(1, min(k, num_tokens))

    def routing(self, logits: torch.Tensor):
        """Expert-Choice routing function."""
        logits = logits.view(-1, self.num_experts)
        N, E = logits.shape

        logits = self.apply_z_loss(logits)

        k = self._k_per_expert(N)

        if self.softmax_order == "softmax_topk":
            scores = torch.softmax(logits, dim=-1, dtype=torch.float32)
            top_w, top_idx = torch.topk(scores.transpose(0, 1), k, dim=-1)
        else:  # "topk_softmax"
            top_w, top_idx = torch.topk(logits.transpose(0, 1), k, dim=-1)
            top_w = torch.softmax(top_w, dim=-1, dtype=torch.float32)

        routing_map = torch.zeros(N, E, dtype=torch.bool, device=logits.device)
        probs = torch.zeros(N, E, dtype=top_w.dtype, device=logits.device)
        expert_idx = (
            torch.arange(E, device=logits.device).unsqueeze(-1).expand_as(top_idx)
        )
        flat_token = top_idx.reshape(-1)
        flat_expert = expert_idx.reshape(-1)
        routing_map[flat_token, flat_expert] = True
        probs[flat_token, flat_expert] = top_w.reshape(-1).to(probs.dtype)
