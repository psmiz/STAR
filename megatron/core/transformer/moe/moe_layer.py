...
from megatron.core.transformer.moe.router import (
    TopKRouter, ReLURouter, STARRouter, ExpertChoiceRouter,
)
...
class BaseMoELayer(MegatronModule, ABC):
    ...
    def __init__(
        ...
        self.router: Union[TopKRouter, ReLURouter, STARRouter, ExpertChoiceRouter] = None
        self.experts = None
...
class MoELayer(BaseMoELayer):
    ...
    def __init__(
        ...
        # Initialize router
        routing_modes = [
            config.moe_relu_routing,
            config.moe_star_routing,
            config.moe_expert_choice_routing,
        ]
        if sum(routing_modes) > 1:
            raise ValueError(
                "At most one of ReLU / STAR / Expert-Choice routing may be enabled."
            )
        if config.moe_relu_routing:
            self.router = ReLURouter(config=self.config, pg_collection=pg_collection)
        elif config.moe_star_routing:
            self.router = STARRouter(config=self.config, pg_collection=pg_collection)
        elif config.moe_expert_choice_routing:
