...
class MoEAlltoAllTokenDispatcher(MoETokenDispatcher):
    ...
    def preprocess(self, routing_map: torch.Tensor) -> torch.Tensor:
        ...
        elif self.config.moe_relu_routing or self.config.moe_expert_choice_routing:
            # ReLU and Expert-Choice routing produce variable per-token counts;
            # use the true routed-token count.
            self.num_out_tokens = num_local_tokens_per_expert.sum()
            self._maybe_update_cuda_sync_point("before_permutation_1")
        else:
