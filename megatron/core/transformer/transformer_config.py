...
class TransformerConfig(ModelParallelConfig):
    ...
    moe_star_routing: bool = False
    """Enable STAR (Adaptive Sparse Mixture of Experts with Gradient-based Assignment) routing."""

    moe_star_gha_lr: float = 2e-5
    """Learning rate for the Generalized Hebbian Algorithm (GHA) in STAR routing. Default is 2e-5."""

    moe_star_fp32_gate: bool = False
    """Use FP32 precision for STAR gate computations."""

    moe_star_gha_only: bool = False
    ...
    """Initial L1 regularization coefficient for ReLU routing."""

    moe_relu_l1_reg_coeff_multiplier: float = 1.2
    """Adaptive update multiplier for the ReLU L1 regularization coefficient."""

    moe_expert_choice_routing: bool = False
    """Enable Expert-Choice routing (Zhou et al. 2022, https://arxiv.org/pdf/2202.09368v2.pdf).
    Each expert picks its top-k tokens; k_per_expert = floor(moe_expert_capacity_factor *
    num_tokens / num_experts). Auto-load-balanced by construction."""

    moe_expert_choice_softmax_order: str = "softmax_topk"
    """Order of softmax/topk in EC routing. 'softmax_topk' applies softmax over experts then
    topk over tokens (per expert). 'topk_softmax' takes topk over tokens first, then softmax
    over the picked logits per expert."""
    ...
    def __post_init__(self):
        ...
        if self.moe_relu_routing:
            if self.num_moe_experts is None:
                raise ValueError("ReLU routing requires num_moe_experts to be set.")
            if self.moe_star_routing:
                raise ValueError("STAR routing and ReLU routing cannot be enabled together.")
            if self.moe_token_dispatcher_type == "flex":
                raise ValueError("ReLU routing does not support the flex token dispatcher.")
            if self.moe_expert_capacity_factor is not None:
                raise ValueError(
                    "ReLU routing does not support token dropping via moe_expert_capacity_factor."
                )
            if self.moe_pad_expert_input_to_capacity:
                raise ValueError(
                    "ReLU routing does not support moe_pad_expert_input_to_capacity."
                )
            if self.moe_router_padding_for_fp8:
                raise ValueError("ReLU routing does not support moe_router_padding_for_fp8.")

        if self.moe_expert_choice_routing:
            if self.num_moe_experts is None:
                raise ValueError("EC routing requires num_moe_experts to be set.")
            if self.moe_relu_routing or self.moe_star_routing:
                raise ValueError(
                    "EC routing cannot be combined with ReLU or STAR routing."
                )
            if self.moe_token_dispatcher_type == "flex":
                raise ValueError(
                    "EC routing does not support the flex (DeepEP) dispatcher."
                )
            if self.moe_pad_expert_input_to_capacity:
                raise ValueError(
                    "EC routing is incompatible with moe_pad_expert_input_to_capacity."
                )
            if self.moe_expert_capacity_factor is None:
                raise ValueError(
                    "EC routing requires --moe-expert-capacity-factor to determine k per expert."
                )
            if self.moe_router_padding_for_fp8:
                raise ValueError("EC routing does not support moe_router_padding_for_fp8.")
            if self.moe_expert_choice_softmax_order not in ("softmax_topk", "topk_softmax"):
                raise ValueError(
                    "Invalid moe_expert_choice_softmax_order: "
                    f"{self.moe_expert_choice_softmax_order}"
                )
