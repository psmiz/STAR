...
def get_model_provider(eval_metric):
    ...
    def model_provider(pre_process=True, post_process=True):
        ...
        # Use mcore models if not using legacy models
        if not args.use_legacy_models:
            from model_provider import model_provider as mcore_model_provider
            from gpt_builders import gpt_builder
            from functools import partial
            return mcore_model_provider(gpt_builder, pre_process, post_process)
        else:
            # Legacy model path
...
def process_batch(batch):
    ...
    attention_mask, _, position_ids = get_ltor_masks_and_position_ids(
        tokens,
        tokenizer.eod,
        tokenizer.eod,  # pad_token (use eod as pad for LAMBADA)
...
def forward_step(batch, model, eval_metric, config):
    ...
    # For single pipeline stage (PP=1), input_tensor is None
    if parallel_state.get_pipeline_model_parallel_world_size() == 1:
        input_tensor = None
    else:
        raise NotImplementedError("Pipeline parallelism not supported for zero-shot evaluation")
