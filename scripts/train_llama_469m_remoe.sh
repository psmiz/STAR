#!/bin/bash
# bash scripts/train_llama_469m_remoe.sh [gpus_per_node] [train_iters] [micro_batch_size] [num_experts] [granularity] [project_name]
...
NUM_EXPERTS=${4:-"8"}
GRANILARITY=${5:-"1"}
PROJECT_NAME=${6:-"remoe_469m_lr4e4_aux_train_B64_E8_G1"}
...
if [ -f "$TRACKER_FILE" ]; then
    ...
    if [ -n "$ITER_DIR" ] && [ -f "${ITER_DIR}/metadata.json" ]; then
        ...
        echo "Resuming from checkpoint path: $CHECKPOINT_PATH"
    else
        echo "No valid distributed checkpoint metadata found at ${CHECKPOINT_PATH}; starting from random init."
    fi
fi
